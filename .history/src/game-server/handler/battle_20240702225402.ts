...
import { AvatarJson, BattleType, JsonData } from 'src/data/loadData';
...
export async function onStartCocoonStageCsReq (
    ...
    if (player.battle_config.battle_type === BattleType.PF) {
        const battle_target: starrail.KNBBHOJNOFF = new starrail.Cnngaoldopi({
            fdfcmhbhnmc: [{
                id: 10001,
                progress: 0,
            }],
    ...
    if (player.battle_config.battle_type === BattleType.AS) {
        const battle_target: starrail.Cnngaoldopi = new starrail.Cnngaoldopi({
            fdfcmhbhnmc: [{
                id: 90005,
                progress: 0,
            }],
