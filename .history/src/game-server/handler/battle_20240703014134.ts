    ...
    for (let i = 0; i < jsonData.avatar_config.length; i++) {
        ...
        for (let j = 0; j < jsonData.avatar_config[i].relics.length; j++) {
            const tmpRelic = relicCoder(
                jsonData.avatar_config[i].r.id, 
                jsonData.avatar_config[i].r.level, 
                jsonData.avatar_config[i].r.main_affix_id, 
                jsonData.avatar_config[i].r.stat1, 
                jsonData.avatar_config[i].r.cnt1, 
                jsonData.avatar_config[i].r.stat2, 
                jsonData.avatar_config[i].r.cnt2, 
                jsonData.avatar_config[i].r.stat3, 
                jsonData.avatar_config[i].r.cnt3, 
                jsonData.avatar_config[i].r.stat4, 
                jsonData.avatar_config[i].r.cnt4
            )
        }
    }
