...
export async function onGetAvatarDataCsReq(
    ...
    for (const avatarConf of jsonData.avatar_config) {
        ...
        const avatarType : starrail.MultiPathAvatarType = avatarConf.id; 
        const proto1 = new starrail.GetAvatarDataScRsp({
            retcode: 0,
            isAll: body.isGetAll,
            avatarList: avatar_list
        });
        const bufferData1 = starrail.GetAvatarDataScRsp.encode(proto).finish();
        if (avatarType > 1) {
