...
export async function onStartCocoonStageCsReq(
    ...
    if (player.battle_config.battle_type === BattleType.PF) {
        const battle_target: starrail.BattleTargetInfo = new starrail.BattleTargetInfo({
            KNBBHOJNOFF: [{
                Id: 10001,
                Progress: 0,
            }],
        });
        battle_info.battleTargetInfo[1] = battle_target;
        for (let i = 2; i <= 4; i++) {
            battle_info.battleTargetInfo[i] = { KNBBHOJNOFF: [] };
        }
        battle_info.battleTargetInfo[5] = {
            KNBBHOJNOFF: [{
                Id: 2001,
                Progress: 0,
            }, {
    ...
    if (player.battle_config.battle_type === BattleType.SU) {
        battle_info.MKMGHEFLFDC.push(new starrail.FOOFENJCLPI({
            DGGCAFCLPFB: player.battle_config.path_resonance_id,
            status: {
                CLIEKBMNBMF: {
                    spCur: 10000,
                    spNeed: 10000,
    ...
    console.log(battle_info.monsterWaveList)
