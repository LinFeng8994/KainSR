...
export async function onPveBattleResultCsReq(
    ...
    const bufferData = starrail.    const bufferData = starrail..encode(proto).finish().encode(proto).finish()


    await player.send(CmdID.    await player.send(CmdID., bufferData);, bufferData);
}