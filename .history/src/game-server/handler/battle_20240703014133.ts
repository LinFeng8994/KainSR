    ...
    for (let i = 0; i < jsonData.avatar_config.length; i++) {
        ...
        for (let j = 0; j < jsonData.avatar_config[i].relics.length; j++) {
            const tmpRelic = relicCoder(
                jsonData.avatar_config[i]..id, 
                jsonData.avatar_config[i]..level, 
                jsonData.avatar_config[i]..main_affix_id, 
                jsonData.avatar_config[i]..stat1, 
                jsonData.avatar_config[i]..cnt1, 
                jsonData.avatar_config[i]..stat2, 
                jsonData.avatar_config[i]..cnt2, 
                jsonData.avatar_config[i]..stat3, 
                jsonData.avatar_config[i]..cnt3, 
                jsonData.avatar_config[i]..stat4, 
                jsonData.avatar_config[i]..cnt4
            )
        }
    }
