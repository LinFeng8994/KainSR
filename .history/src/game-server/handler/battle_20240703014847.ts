...
export async function onStartCocoonStageCsReq(
    ...
    for (let i = 0; i < jsonData.avatar_config.length; i++) {
        ...
        if (jsonData.avatar_config[i].use_technique) {
            ...
            buff.dynamicValues["SkillIndex"] value: 0 });
    ...
    battle.BOJHPNCAKOP = jsonData.battle_config.monster_wave.items.length;

    // Monster handler
    for (const wave of jsonData.battle_config.monster_wave.items) {
    ...
    // Stage blessings
    for (const blessing of jsonData.battle_config.blessings.items) {
        const targetIndexList = new starrail.ArrayList<number>();
        targetIndexList.push(0);
        const buff = new starrail.BattleBuff({
            id: blessing,
            level: 1,
            ...
            waveFlag: 0xffffffff,
            targetIndexList: targetIndexList,
            dynamicValues: new starrail.ArrayList<starrail.BattleBuff.DynamicValuesEntry>(),
        });
        buff.dynamicValues.push({ key: { Const: "SkillIndex" }, value: 0 });
        battle.buffList.push(buff);
    }

    // PF/AS scoring
    const BattleTargetInfo = starrail.SceneBattleInfo.BattleTargetInfo;
    battle.battleTargetInfo = new starrail.ArrayList<BattleTargetInfo>();

    // Target hardcode
    let pfTargetHead = new starrail.BattleTargetList();
    pfTargetHead.knbbhojnoff = new starrail.ArrayList<starrail.BattleTarget>();
    pfTargetHead.knbbhojnoff.push({ id: 10002, progress: 0, totalProgress: 0 });

    let pfTargetTail = new starrail.BattleTargetList();
    pfTargetTail.knbbhojnoff = new starrail.ArrayList<starrail.BattleTarget>();
    pfTargetTail.knbbhojnoff.push({ id: 2001, progress: 0, totalProgress: 0 });
    pfTargetTail.knbbhojnoff.push({ id: 2002, progress: 0, totalProgress: 0 });

    let asTargetHead = new starrail.BattleTargetList();
    asTargetHead.knbbhojnoff = new starrail.ArrayList<starrail.BattleTarget>();
    asTargetHead.knbbhojnoff.push({ id: 90005, progress: 0, totalProgress: 0 });
    ...
    switch (battle.stageId) {
        ...
        // Add more cases as needed
        case 30307000:
            battle.battleTargetInfo.push({ key: 1, value: pfTargetHead });
            for (let i = 2; i <= 5; i++) {
                battle.battleTargetInfo.push({ key: i });
            }
            battle.battleTargetInfo.push({ key: 5, value: pfTargetTail });
            break;
        // AS
        case 420100:
        // Add more cases as needed
        case 420200:
            battle.battleTargetInfo.push({ key: 1, value: asTargetHead });
            break;
        default:
            break;
    }
