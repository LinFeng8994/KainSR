...
export async function onReplaceLineupCsReq(
    ...
    for (let i = 0; i < body.IPHNMDOIFON..avatar_config.length; i++) {
        const tmpAvatar : starrail.LineupAvatar = new starrail.LineupAvatar();
        switch (body.IPHNMDOIFON..avatar_config[i].id) {
            case 8001:
            case 8002:
            case 8003:
            case 8004:
            case 8005:
            case 8006:
                tmpAvatar.id = body.IPHNMDOIFON..avatar_config[i].id;
            default:
                tmpAvatar.id = body.IPHNMDOIFON..avatar_config[i].id;
        }
        tmpAvatar.slot = i;
        tmpAvatar.satiety = 0;
        tmpAvatar.hp = body.IPHNMDOIFON..avatar_config[i].hp * 100;
        tmpAvatar.sp = {
            spCur: jsonData.avatar_config[i].sp * 100,
            spNeed: 10000
        }
