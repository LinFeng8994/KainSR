...
export async function onGetAvatarDataCsReq(
    ...
    for (let i = 0; i < UNLOCKED_AVATARS.length; i++) {
        const avatar = new starrail.Avatar({
            ...
            promotion: 6,
            rank: 6,
            iDGAGNIFBNF: Array.from({ length: 5 }, (_, index) => index + 1)
        });
    ...
    for (const avatarConf of jsonData.avatar_config.items) {
        const avatar = new starrail.Avatar({
            baseAvatarId: mapAvatarId(avatarConf.id),
            ...
            skilltreeList: getSkills(avatarConf.id)
        });
        avatar_list.push(avatar);

        const avatarType = avatarConf.id; // Assuming enumFromInt(avatarConf.id) logic

        if (avatarType > 1) {
            await player.send(CmdID.CmdSetAvatarPathScRsp, new starrail.SetAvatarPathScRsp({
                retcode: 0,
                avatarId: avatarType
            }));
        }
