...
export async function onGetBagCsReq (
    ...
    for (const avatarConf of jsonData.avatar_config) {
        ...
        for (const relic of avatarConf.relics) {
            ...
            tmpRelic.subAffixList.push(new starrail.RelicAffix({ affixId : relic.stat1, cnt : cnt1, step : 3 }));
            tmpRelic.subAffixList.push(new starrail.RelicAffix({ affixId : relic.stat2, cnt : cnt2, step : 3 }));
            tmpRelic.subAffixList.push(new starrail.RelicAffix({ affixId : relic.stat3, cnt : cnt3, step : 3 }));
            tmpRelic.subAffixList.push(new starrail.RelicAffix({ affixId : relic.stat4, cnt : cnt4, step : 3 }));
