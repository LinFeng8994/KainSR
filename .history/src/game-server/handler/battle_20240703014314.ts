...
export async function onStartCocoonStageCsReq(
    ...
    for (let i = 0; i < jsonData.avatar_config.length; i++) {
        ...
        const lc = new starrail.BattleEquipment({
           id : tmpAvatar.avatarType.lightcone.id,
           rank : tmpAvatar.avatarType.lightcone.rank,
           level : tmpAvatar.avatarType.lightcone.level,
           promotion : tmpAvatar.avatarType.lightcone.promotion,
        });
