    ...
    for (const avatarConf of jsonData.avatar_config) {
        ...
        for (const relic of avatarConf.relics) {
            const tmpRelic : starrail.Relic= new starrail.Relic({
                APAMKMIGAOP : inputid, // tid
                main_affix_id : inputmain_affix_id,
                unique_id : generatornextId(),
                exp : 0,
                OKMNCEFGIMF : avatarConfid, // base avatar id
                HDHDLMJIFAO : true, // lock
                level : inputlevel,
                sub_affix_list : ArrayList(protocolRelicAffix).init(allocator),
            });
        }
    }
}
