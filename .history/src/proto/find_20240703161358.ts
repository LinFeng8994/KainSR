...
const targetMessages = messages.filter(message => {
    const boolField = message.fields.find(field => field.type === 'uint32');
    const boolField1 = message.fields.find(field =>field.type === 'uint64');
    ...
    return boolField && boolField1   && message.fields.length == 2;
});
