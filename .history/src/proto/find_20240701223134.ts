...
const targetMessages = messages.filter(message => {
    const boolField = message.fields.find(field => field.type === 'repeated' && field.index === 15);
    const boolField1 = message.fields.find(field => field.type === 'repeated' && field.index === 11);
    const boolField2= message.fields.find(field => field.type === 'repeated' && field.index === 1);
    ...
    return boolField && boolField1 && boolField2  &&message.fields.length == 3;
});
