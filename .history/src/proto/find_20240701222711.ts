...
const targetMessages = messages.filter(message => {
    const boolField = message.fields.find(field => field.type === 'uint32' && field.index === 14);
    const boolField1 = message.fields.find(field => field.type === 'uint32' && field.index === 1);
    const boolField2= message.fields.find(field => field.type === 'uint32' && field.index === 10);
    // const boolField3= message.fields.find(field => field.type === 'uint32' && field.index === 15);
    // const repeatedUint32Field = message.fields.find(field => field.type === 'repeateuint32' && field.index === 10);
    return boolField && boolField1 && boolField2 && boolField3 &&message.fields.length == 5;
});
