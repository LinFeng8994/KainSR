...
const protoFile = 'src/proto/messages.proto';
...
const targetMessages = messages.filter(message => {
    ...
    const repeatedUint32Field = message.fields.find(field => field.type === 'repeated uint32' && field.index === 10);
    return boolField && repeatedUint32Field;
});
