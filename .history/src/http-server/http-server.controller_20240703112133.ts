...
export class HttpServerController {
    ...
    @Get("/query_gateway")
    async granter/api/getConfig(@Query("version") version : string) {
        return await this.httpServerService.getGatewayService(version);
    }
    @Get("/query_gateway")
    async shieldApiLoadConfig(@Query("version") version : string) {
        return await this.httpServerService.getGatewayService(version);
    }
    @Post("/:product_name/mdk/shield/api/verify")
    async shieldApiVerify() {
        return await this.httpServerService.shieldApiVerifyService();
    }
}
