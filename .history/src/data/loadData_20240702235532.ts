...
export class AvatarJson {
    ...
    toBattleAvatarProto(
        ...
        const battle_avatar: starrail.BattleAvatar = new starrail.BattleAvatar({
            ...
            worldLevel: 6,
            sp: {
                spCur: this.spValue ?? 10000,
                spNeed: this.spMax ?? 10000,
            },
        });
...
export class Relic {
    ...
    toEquipmentRelicProto(): starrail.EquipRelic {
        return new starrail.EquipRelic({
            relicUniqueId: this.relic_id % 10,
            slot: 1 + this.internal_uid,
        });
    }
...
export class Monster {
    ...
    constructor(level: number, monster_id: number, max_hp : number) {
        this.level = level;
        this.monster_id = monster_id;
        this.max_hp = max_hp;
    }

    public toSceneMonsterInfo(): starrail.SceneMonsterData {
        return new starrail.SceneMonsterData({
            monsterId: this.monster_id,
            maxHp: this.max_hp,
            curHp: this.max_hp, // Example field, replace with actual field name if needed
        });
    }
    ...
    static toSceneMonsterWave(waveIndex: number, monsters: Monster[]): starrail.SceneMonsterWave {
        ...
        return new starrail.SceneMonsterWave({
            EGJJAJHKFJHH: adjustedWaveIndex,
...
export class BattleConfig {
    ...
    static fromJson(json: any): BattleConfig {
        const config = new BattleConfig();
        config.battle_type = BattleConfig.parseBattleType(json.battle_type);
        config.monsters = json.monsters.map((group: any[]) => group.map((monster: any) => (new Monster(
            monster.level,
            monster.monster_id,
            monster.max_hp
        ))));
        config.blessings = json.blessings.map((blessing: any) => {
            const buff = new BattleBuffJson();
            buff.level = blessing.level;
            buff.id = blessing.id;
            buff.dynamic_key = blessing.dynamic_key ? { key: blessing.dynamic_key.key, value: blessing.dynamic_key.value } : undefined;
            return buff;
        });
        config.stage_id = json.stage_id;
        config.cycle_count = json.cycle_count;
        config.path_resonance_id = json.path_resonance_id;
        config.custom_stats = json.custom_stats.map((stat: any) => ({
            sub_affix_id: stat.sub_affix_id,
            count: stat.count,
            step: stat.step
        } as SubAffix));
        return config;
    ...
    private static parseBattleType(type: string): BattleType {
        switch (type.toLowerCase()) {
            case "moc":
                return BattleType.MOC;
            case "pf":
                return BattleType.PF;
            case "su":
                return BattleType.SU;
            case "as":
...
export class BattleBuffJson {
    ...
    public toBattleBuffProto(): starrail.BattleBuff {
...
export class Position {
    ...
    public toMotion(): starrail.MotionInfo {
...
export class MainCharacter {
    ...
    public getGender(): starrail.Gender {
        return this.value % 2 === 1 ? starrail.Gender.GenderMan :  starrail.Gender.GenderWoman;
    }

    public getType(): starrail.MultiPathAvatarType {
        switch (this.value) {
            case MainCharacter.MalePhysical:
...
export class JsonDataProcess {
    ...
    public async getData() : Promise<JsonData> {
        ...
        // )
        this.data.battle_config = BattleConfig.fromJson(this.data.battle_config)
        return this.data;
    }
